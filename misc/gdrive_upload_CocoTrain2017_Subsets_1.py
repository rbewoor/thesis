## -----------------------------------------------------------------------------
## 75000 - 80000       1a8BKBcdLj0jGZyyUetvAzNJt4nk-DKwr
## -----------------------------------------------------------------------------
### Goal: Upload files from local (laptop) folder to specific Google drive folder-
##        Use the CL parameters to specifiy which files
##        NOTES:
##              1) Reload the os.listdir of the Google Drive from pickle file. This is used and the start and end indexex slice the output of it.
...
##              4) The target folder on Google Drive is specified using the folder-id (end of the url when you open that folder). This is accepted as a command line parameter.
...
## 3) -fid    :folder-id of the target folder
## 4) -sf     :status frequency paramter
##             print a statement showing how many processed till then
## -----------------------------------------------------------------------------
## Usage example:
##    python3 gdrive_upload_CocoTrain2017_1.py -sidx 75000 -eidx 80000 -fid '1a8BKBcdLj0jGZyyUetvAzNJt4nk-DKwr' -sf 10
...
argparser.add_argument(
    '-fid',
    '--folderid',
    help='folder-id of the target folder on gdrive')
...
def my_upload_file(_drive_service, _source_file_with_full_path, _gdrive_target_folder_id = None):
    try:
...
def _main_(args):
    ...
    target_fid   = args.folderid             # -fid  parameter
    ...
    GDRIVE_TARGET_FOLDER_ID = target_fid

    ## reload from pickle file - output of os.listdir(GDRIVE folder of 118k images)
    with open(
        '/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Code/ModelsRuns/ImgCap/SavedData/' + 'gdrive_coco_train2017_118287_listdir.pkl', 'rb') as handle:
        gdrive_coco_train2017_118287_listdir = pickle.load(handle)
    
    files_to_upload_arr = gdrive_coco_train2017_118287_listdir[start_idx: end_idx]
    files_to_upload_arr = [SOURCE_FOLDER_LOCAL + f for f in files_to_upload_arr]
    
    print(f"\nfiles_to_upload_arr is ready from pickled info, its length = {len(files_to_upload_arr)}")

    #print(f"\nFew entries = \n{files_to_upload_arr[:5]}")
    
    print(f"\n\nGoing to start attempting upload from the files array\nStart index = {start_idx} , End index = {end_idx}\nTarget Gdrive folder-id = {GDRIVE_TARGET_FOLDER_ID}")
