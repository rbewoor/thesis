...
class c_queryImgSelection_root_window:
    def __init__(self, _DEBUG_SWITCH, _query_num, _each_query_result, _index_positions_to_remove_this_query, _num_candidate_images_before_selection_began):
        ...
        if _num_candidate_images_before_selection_began == 0:
            self.root.title(f"Root Window - Image Selection - Query number {_query_num} - No Images to Display")
        else:
            self.root.title(f"Root Window - Image Selection - Query number {_query_num} - Click button to Proceed")
        ...
        ## if no images to display in grid, disable the button to proceed
