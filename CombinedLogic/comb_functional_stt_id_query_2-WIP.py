...
###          interface and Deselect any images. Ensuring a maximum of 5 images are retained from the
###          set of up to 20 images returned per query to Neo4j. Thus reduce the total images from
###          up to 60 images, to a maximum of 15 images. User may Deselect ALL the images from the 
###          results of a query, if they find none of the images are correct.
## 
##  1) STT part:
...
## 
##  4) Graphical user interaction based Deselection of images obtained from Neo4j query stage.
##        Allow user to DESELECT images returned from the Neo4j query stage.
...
def query_image_selection_via_gui_functionality(_query_neo4j_module_results, _DEBUG_SWITCH):
    '''
    High level module to present images returned by neo4j query via a graphical user interface.
        Allow Deselection of images for two purposes:
            1) that are wrongly picked up (as object actually not present)
            2) Maximum limit of images per query is 5 to proceed to next stage. The Neo4j query can return
               up to 20 images.
        Allow Enlargement of images to study them more clearly to decide on Deselection.
        After Enlarged image is shown, do an optional inference again to see the output from Object Detection
            model. Thus providing more information to user to make Deselection decision.
    VARIABLES PASSED:
          1) Data structure containing results of queries to Neo4j database; consisting of image name and the
    ...
    RETURNS:
          1) Return code
                ...
                non 0      => some problem
          2) Message if return code is non-zero, else will be None
          3) Results array - same data struture as the input but with the culled images.
...
def execute_control_logic(args):
    ...
    ## do the query results images selection via gui logic processing
    if query_neo4j_logic_RC == 0:
        print(f"\n\n-------------------------------------------------------------------")
        print(f"-------------------------------------------------------------------")
        print(f"  STARTING EXECUTION OF NEO4J QUERY RESULTS IMAGE SELECTION LOGIC  ")
        print(f"-------------------------------------------------------------------")
        print(f"-------------------------------------------------------------------\n\n")
        query_image_selection_logic_RC, query_image_selection_logic_msg, query_image_selection_module_results = query_image_selection_via_gui_functionality(query_neo4j_module_results, DEBUG_SWITCH)
    else:
        print(f"\n\nFATAL ERROR: Identify keywords logic failed. Return code = {query_neo4j_logic_RC}\n\tMessage = {query_neo4j_logic_msg}")
        print(f"\n\nPremature Exit.\n\n")
        exit(200)
