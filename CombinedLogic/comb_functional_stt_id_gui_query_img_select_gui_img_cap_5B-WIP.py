...
def query_neo4j_db(_each_inner_list, _in_limit):
    ...
    """
    ## build the query and execute -- version without any HAS reln score threshold and without source dataset restrictions
    ...
    stmt3_one_objects = r'MATCH (o1:Object)--(i:Image) ' + \
        ...
        r'LIMIT $in_limit'
    """
    ## build the query and execute -- HAS reln score > 90.0 and source dataset either flickr or coco_2017_test
...
def gui_img_cap_show_results_functionality(img_cap_inference_module_results, DEBUG_SWITCH):
    return (0, None, None)
...
## ********** -------------  **************** ------------------ ****************
    ...
    IMG_CAP_RESULTS_SAVE_TO_FILE = r'/home/rohit/PyWDUbuntu/thesis/combined_execution/ImgCapNoGui/img_cap_no_gui_results.txt'
...
## ********** -------------  **************** ------------------ ****************
    ...
    print_and_log(myStr, "info")
    myStr = None
    ## save the results to file if all ok
    if img_cap_inference_logic_RC == 0:
        try:
            #IMG_CAP_RESULTS_SAVE_TO_FILE is defined above
            with open(IMG_CAP_RESULTS_SAVE_TO_FILE, 'w') as f:
                    json.dump(img_cap_inference_module_results, f)
            myStr = f"\n\nImage Captioning results without GUI saved to file here :\n{IMG_CAP_RESULTS_SAVE_TO_FILE}\n\n"
            print_and_log(myStr, "info")
            myStr = None
        except Exception as imgCapResultsSaveToFileError:
            print(f"\nFATAL ERROR: Problem creating the output file for all words pos info.\nError message: {imgCapResultsSaveToFileError}\n")
...
## ********** -------------  **************** ------------------ ****************
    ...
    if img_cap_inference_logic_RC == 0:
        ...
        print_and_log(myStr, "info")
        myStr = None
        gui_img_cap_show_results_logic_RC, gui_img_cap_show_results_logic_msg, gui_img_cap_show_results_module_results = gui_img_cap_show_results_functionality(img_cap_inference_module_results, DEBUG_SWITCH)
        myStr = "\n".join([
