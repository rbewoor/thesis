...
### Goal: Accept user input via mic and save as 3 sentences and save each as a Wav file.
###       NOTE: Mic input is optional and user can choose to run program by executing from Stage 2 directly.
###       Run the saved Wave files through STT block to get transcriptions for each of the three input files.
###       Then do NLP processing on the transcriptions to extract the keywords.
###       Then query the Neo4j database to return images containing the objects of interest.
###          Per query, limit results to maximum 20 images. These are the "candidate images".
###       Present candidate images via GUI to user to inspect and/or deselect images.
###          Per Query, a maximum of 5 images can be Selected.
###          Thus reduce the total images from up to 60 (20 x 3), to maximum 15 (5 x 3).
###          User may Deselect ALL the images if none make sense.
###       Then perform Image Captioning on the selected images.
###          Beam search with width=3 is used in inference.
###       Present the images and captions via GUI to user to inspect, edit captions and deselect
###          any images.
###       Then, use these captions as seed for the Story Geneation logic.
## -----------------------------------------------------------------------------
##  STAGE 1) Mic Input:
##     Accept 3 sentences as user input from mic.
##     Outputs:
##        a) 3 wav files to be stored in CLA parameter location
##
##  STAGE 2) STT part:
##     Use Deepspeech model to run inference on the input wav files.
##     Display the inferences via a GUI window - but no editing allowed.
##     Outputs:
##        a) Intermediate data:
##           for use by the identify keywords logic
## 
##  STAGE 3) Identify keywords and Deselect words via GUI
...
## 
##  STAGE 4) Query Neo4j for images matching the keywords
...
## 
##  STAGE 5) GUI based Deselection of candidate images obtained from Neo4j query stage.
...
## 
##  STAGE 6) Image captioning on the images selected by the user.
##        Setup a the Encoder-Decoder model to perform image captioning and insert the captions in data structure.
##        Logic:
##           a) The encoder is a Keras Google Inception-v3 model pre-trained on Imagenet.
##           b) Decoder is a single layer 256 cell LSTM with merge architecture and the weights are populated from a locally saved file.
...
##           a) None
## 
##  STAGE 7) GUI presentation of the Images and their captions.
##        Show the images and their captions to the user with option to edit the captions
##        Logic:
...
## python3 comb_functional_stt_id_gui_query_img_select_gui_img_cap_6D-WIP.py -micYorN "N" -sw_here "/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/audio/wavs/fromMic/" -file4STT "/home/rohit/PyWDUbuntu/thesis/combined_execution/SttTranscribe/mic_input_wavfiles_1.txt" -opfileallposinfo "/home/rohit/PyWDUbuntu/thesis/combined_execution/IdElements/all_words_pos_info_1.txt" -logfileloc "./LOG_comb_functional_stt_id_gui_query_img_select_gui_img_cap_6D_WIP.LOG"
## -----------------------------------------------------------------------------
...
###############################################################################################################
## -----------------------  MIC INPUT LOGIC STARTS              MIC INPUT LOGIC  STARTS ----------------------- 
###############################################################################################################
def listen_only():
...
def listen_and_save_file(_specs_listen_list, _save_wav_here, _wav_files_arr, _DEBUG_SWITCH):
    ...
    ## loop to capture 3 user inputs as individual wav files
    for idx in range(3):
        new_wav_file_path = ''.join([
            _save_wav_here,
            'mic_wav_file',
            str(idx+1),
            '.wav'
        ...
        ## Wait till user confirms ready or Quit
        input(f"\nPress any key to start recording {idx+1} of 3, Note: you MUST provide 3 total recordings...")
        ...
        while True:
            for i in range(0, int(FS / CHUNCK * _append_every_secs)):
                ...
                frames.append(data)
            vol = max(data)
            if (vol < _min_vol) and ( (time.time() - start_time) > _min_secs ):
                myStr = '\n'.join([
                    f"\nStopped recording: in last {_append_every_secs} seconds, volume was below threshold",
                    f"vol ({vol}) < threshold value ({_min_vol})\n"
                    ])
                print_and_log(myStr, "info")
                myStr = None
                break
        ...
        ## save new wav file
        wf = wave.open(new_wav_file_path, 'wb')
        ...
        ## add entry in list
        _wav_files_arr.append(new_wav_file_path)

        myStr = '\n'.join([
            f"\nSaved Wav file here: {new_wav_file_path}"
            f"Recording length = {rec_secs:.2f} seconds\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
...
def mic_input_save_as_wav_functionality(_save_wav_here, _wavlocfile, _DEBUG_SWITCH):
    '''
    High level module to execute the STT processing of wav files and get the transcriptions.
    Wav files saved as per requirements of Deepspeech (16kHz, 16-bit, Mono-channel)
    Will listen for a minimum number of seconds = LISTEN_MIN_SECS
    Will listen for a maximum number of seconds = LISTEN_MAX_SECS
    Stops listening if no input is detected for DATA_APPEND_SECS seconds after LISTEN_MIN_SECS.
          Detected by checking volume of just recorded window against preset threshold = MIN_VOLUME.
    VARIABLES PASSED:
    ...
    '''
    ## check valid input for the folder location to save wav files
    if _save_wav_here[-1] != r'/':
        _save_wav_here = _save_wav_here + r'/'
    if not os.path.isdir(_save_wav_here):
        return_msg = f"\nFATAL ERROR: Folder to save wav files to does not exist:\n{_save_wav_here}\nRC=100"
    ...
    ## empty list to hold paths to wav files being created
    wav_files_for_stt_arr = []
    listen_and_save_file(specs_listen_list, _save_wav_here, wav_files_for_stt_arr, _DEBUG_SWITCH)

    ## save entries to file - place single quotes around filename first
    #write_string = '\n'.join(["'" + f + "'" for f in wav_files_for_stt_arr ]) + '\n'
    
    ## save entries to file - NO single quotes around filename first
    write_string = '\n'.join(wav_files_for_stt_arr) + '\n'
    ...
    myStr = '\n'.join([
        f"\nSaved {len(wav_files_for_stt_arr)} wav files to folder: {_save_wav_here}",
        f"Wav files created are: {wav_files_for_stt_arr}"
        f"\nCreated file for STT block processing here: {_wavlocfile}"
        ])
    print_and_log(myStr, "info")
    myStr = None
...
###############################################################################################################
## -------------------------  MIC INPUT LOGIC ENDS              MIC INPUT LOGIC  ENDS ------------------------- 
###############################################################################################################
...
def stt_transcribe_functionality(_wavlocfile, _DEBUG_SWITCH):
    ...
    #     
    ## display the inferences via a gui window

    ## root window
    root_stt_inference = tk.Tk()
    root_stt_inference.title(f"STT model inferences")
    ## label for instructions
    msg_instructions_for_root_stt_inference = "".join([
        f"                               ----------------------------------",
        f"\n                               -----      INSTRUCTIONS      -----",
        f"\n                               ----------------------------------",
        f"\n",
        f"\n",
        f"\nTranscription outputs from the STT model.",
        f"\n",
        f"\n\t1.  Just for display purposes.",
        f"\n",
        f"\n\t2.  No editing possible. Data will be passed as is for downstream processing."
        f"\n",
        f"\n\t3.  Once ready, simply close this window to continue processing.",
        f"\n"
    ])
    lbl_root_instructions = tk.Label(
        master=root_stt_inference,
        text=msg_instructions_for_root_stt_inference,
        bg="blue", fg="white",
        justify=tk.LEFT,
        width=130, height=15,
        relief=tk.FLAT
    ...
    ## label for wav file 1 path
    lbl_wav1_path = tk.Label(
        master=frm_wav1,
        text=f"Wav file 1 path = {wavfiles_arr[0]}",
        bg="grey", fg="white",
        relief=tk.FLAT
        )
    lbl_wav1_path.configure(
        width=( len(lbl_wav1_path["text"]) + 10),
        height=3
    )
    ## label for STT output of wav file 1
    lbl_wav1_stt_output_text = "\n".join( ["Inference on next line =", deepspeech_inferences_arr[0]] )
    lbl_wav1_stt_output = tk.Label(
        master=frm_wav1,
        text=lbl_wav1_stt_output_text,
        justify=tk.LEFT,
        bg="yellow", fg="black",
        relief=tk.FLAT
        )
    lbl_wav1_stt_output.configure(
        width=( 20 + max( [len(line_text) for line_text in lbl_wav1_stt_output_text.split("\n")] )  ),
        height=4
    )

    ## frame for wav file 2
    frm_wav2 = tk.Frame(
        master=root_stt_inference,
        relief=tk.SUNKEN,
        highlightthickness = 5,
    ...
    ## label for wav file 2 path
    lbl_wav2_path = tk.Label(
        master=frm_wav2,
        text=f"Wav file 2 path = {wavfiles_arr[1]}",
        bg="grey", fg="white",
        relief=tk.FLAT
        )
    lbl_wav2_path.configure(
        width=( len(lbl_wav2_path["text"]) + 10),
        height=3
    )
    ## label for STT output of wav file 2
    lbl_wav2_stt_output_text = "\n".join( ["Inference on next line =", deepspeech_inferences_arr[1]] )
    lbl_wav2_stt_output = tk.Label(
        master=frm_wav2,
        text=lbl_wav2_stt_output_text,
        justify=tk.LEFT,
        bg="yellow", fg="black",
        relief=tk.FLAT
        )
    lbl_wav2_stt_output.configure(
        width=( 20 + max( [len(line_text) for line_text in lbl_wav2_stt_output_text.split("\n")] )  ),
        height=4
    )

    ## frame for wav file 3
    frm_wav3 = tk.Frame(
        master=root_stt_inference,
        relief=tk.SUNKEN,
        highlightthickness = 5,
    ...
    ## label for wav file 3 path
    lbl_wav3_path = tk.Label(
        master=frm_wav3,
        text=f"Wav file 3 path = {wavfiles_arr[2]}",
        bg="grey", fg="white",
        relief=tk.FLAT
        )
    lbl_wav3_path.configure(
        width=( len(lbl_wav3_path["text"]) + 10),
        height=3
    )
    ## label for STT output of wav file 3
    lbl_wav3_stt_output_text = "\n".join( ["Inference on next line =", deepspeech_inferences_arr[2]] )
    lbl_wav3_stt_output = tk.Label(
        master=frm_wav3,
        text=lbl_wav3_stt_output_text,
        justify=tk.LEFT,
        bg="yellow", fg="black",
        relief=tk.FLAT
        )
    lbl_wav3_stt_output.configure(
        width=( 20 + max( [len(line_text) for line_text in lbl_wav3_stt_output_text.split("\n")] )  ),
        height=4
    )
    
    ## pack it all
    lbl_root_instructions.pack(padx=20, pady=50)
    lbl_wav1_path.pack(padx=15, pady=15)
    lbl_wav1_stt_output.pack(padx=15, pady=15)
    lbl_wav2_path.pack(padx=15, pady=15)
    lbl_wav2_stt_output.pack(padx=15, pady=15)
    lbl_wav3_path.pack(padx=15, pady=15)
    lbl_wav3_stt_output.pack(padx=15, pady=15)
    frm_wav1.pack(padx=10, pady=10)
    frm_wav2.pack(padx=10, pady=10)
    frm_wav3.pack(padx=10, pady=10)
    
    ## show the window
    root_stt_inference.mainloop()
...
###############################################################################################################
## -----------   IMAGE CAPTONING INFERENCE STARTS                  IMAGE CAPTONING INFERENCE STARTS -----------
###############################################################################################################
...
def img_cap_inference_functionality(_id_elements_module_results, _gui_candidate_image_selection_module_results, _imgcap_inference_beam_width, _DEBUG_SWITCH):
    """
    ...
    SAVED_WEIGHTS_PATH = r'/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Code/ModelsRuns/ImgCap/SavedData/Thesis_ImgCap_Weights_In_Run3/Decoder_Run_3_Wt_ep_18.h5'
    ...
    img_cap_rc = 0
    return (img_cap_rc, None, data_for_img_cap_functionality)

###############################################################################################################
## -----------   IMAGE CAPTONING INFERENCE ENDS                      IMAGE CAPTONING INFERENCE ENDS -----------
###############################################################################################################
...
class c_imgCap_wnd_grid_window:
    def __init__(self, _root, _key_elements_info, _selected_imgs_info, _query_num, _imgCap_deselected_positions, _DEBUG_SWITCH):
        self.root = _root
        ...
        ## NOTE: This row count is only for  thumbnail image, path, caption, enlarge, edit buttons rows
        ##       NOT for Confirm button and Label for current count.
        ##           They will be referenced by using row count and indexing further down automatically.
        ## IMPORTANT: ENSURE #rows is multiple of 3 coz the logic in window grid array holding buttons/labels expects this
        ##
        ## 3 for image + 1 for Enlarge + 1 for Edit caption. NOTE: Path and Caption will span last two columns
        ## Logical Row:  first  row = 3 for image + 1 for path    fixed label + 1 for path    actual
        ## Logical Row:  second row = 3 for image + 1 for caption fixed label + 1 for caption actual
        ## Logical Row:  third  row = 3 for image + 1 combined for Enlarge and/or Edit caption button
        ## This set of 3 rows for each logical set repeats for 5 possible images. Thus 3 x 5 = 15
        self.n_rows = 15  ## MUST BE DIVISIBLE BY 3
        ...
        for r_idx in range(self.n_rows):
            ...
            if r_idx % 3 == 0:
                ...
                ## associated enlarge and/or edit caption button - pos 5 in inner list
                temp_row_data.append(
                    tk.Button(
                        master=self.wnd_grid,
        ...
        ##     thumbnail button, path label, caption label, enlarge/edit caption button
        ...
        for r_idx, logical_set_for_image in enumerate(self.grid_buttons_arr):
            for c_idx, button_or_label in enumerate(logical_set_for_image):
                ...
                ## 5                  enlarge and/or edit caption button
                if c_idx == 0:
                    ## image button
                    button_or_label.grid(
                        row=r_idx * 3, column=0,
    ...
    def imgCap_do_confirm_selections_processing(self):
        ...
        for r_idx, logical_set_for_image in enumerate(self.grid_buttons_arr):
            img_button_info, _, _, _, _, _ = logical_set_for_image ## only interested in the first entry
...
def imgcap_generic_show_grid_selection_window(_root, _key_elements_info, _selected_imgs_info, _query_num, _imgCap_deselected_positions, _DEBUG_SWITCH):
    if _DEBUG_SWITCH:
        print(f"\nEntered the show grid selection window for img captioning\n")
    
    o_imgCap_wnd_grid_window = c_imgCap_wnd_grid_window(_root, _key_elements_info, _selected_imgs_info, _query_num, _imgCap_deselected_positions, _DEBUG_SWITCH)
...
class c_imgCap_enlargeEdit_window():
    def __init__(self, _o_wnd_grid, _r_idx, _c_idx, _DEBUG_SWITCH):
        ...
        self.txt_editable_caption = tk.Text(
            master=self.wnd_enlarged_img,
            width = len(self.caption_editable) + 5,
        ...
        self.lbl_edit_caption_message = tk.Label(
            master=self.wnd_enlarged_img,
            text="Optional: You may EDIT the caption in the BLUE box above and then click the CONFIRM button below",
            bg="grey", fg="white",
            relief=tk.SUNKEN,
            borderwidth=5
            )
        ...
        ## pack the widgets
        self.lbl_enlarged_image.pack(padx=15, pady=15)
        self.lbl_orig_caption.pack(padx=10, pady=10)
        self.txt_editable_caption.pack(padx=20, pady=20)
        self.lbl_edit_caption_message.pack(padx=10, pady=10)
        self.btn_confirm_enlarged_window_done.pack(padx=50, pady=50)
        return
    ...
    def imgcap_generic_enlarge_edit_caption_window_confirm(self, _o_wnd_grid, _r_idx, _DEBUG_SWITCH):
        ...
        if self.caption_orig == self.caption_editable:
            ...
            _o_wnd_grid.grid_buttons_arr[_r_idx][4].configure(
                width = len(_o_wnd_grid.grid_buttons_arr[_r_idx][4]["text"]) + 10,
                bg="yellow",fg="black"
            )
...
class c_queryImgCapGuiOneQuery():
    def __init__(self, _DEBUG_SWITCH, _query_num, _key_elements_info, _selected_imgs_info, _imgCap_deselected_positions):
        self.DEBUG_SWITCH = _DEBUG_SWITCH
        self.query_num = _query_num
        self.key_elements_info = _key_elements_info
        self.selected_imgs_info = _selected_imgs_info
        ...
        self.msg_instructions_for_root = "".join([
            ...
            f"\n\t       Optional: There you may edit the caption.",
...
def gui_img_cap_functionality_for_one_query(_DEBUG_SWITCH, _query_num, _key_elements_info, _selected_imgs_info, _imgCap_index_positions_to_remove_this_query):
    if _DEBUG_SWITCH:
        print(f"\n\nquery = {_query_num}\nkeyEleInfo = {_key_elements_info}\nselecImgInfo = {_selected_imgs_info}\n\n")
    ## make a root window and show it
    o_queryImgCapGuiOneQuery_root_window = c_queryImgCapGuiOneQuery(_DEBUG_SWITCH, _query_num, _key_elements_info, _selected_imgs_info, _imgCap_index_positions_to_remove_this_query)
    o_queryImgCapGuiOneQuery_root_window.root.mainloop()

def gui_img_cap_show_gui_functionality(_img_cap_inference_module_results, _DEBUG_SWITCH):
    if _DEBUG_SWITCH:
    ...
    for query_num, query_img_cap_results in enumerate(copy_img_cap_inference_module_results):
        num_imgCap_candidate_images_before_selection_began = len(query_img_cap_results['selected_images'])
        imgCap_index_positions_to_remove_this_query = []
        gui_img_cap_functionality_for_one_query(_DEBUG_SWITCH, query_num+1, query_img_cap_results['key_elements'], query_img_cap_results['selected_images'], imgCap_index_positions_to_remove_this_query)
    ...
    for each_query_result, each_query_pos_remove_list in zip(copy_img_cap_inference_module_results, imgCap_index_positions_to_remove_all_queries):
        final_img_cap_inference_module_results.append({'key_elements': [], 'selected_images': []})
        final_img_cap_inference_module_results[-1]['key_elements'] = each_query_result['key_elements']
        final_img_cap_inference_module_results[-1]['selected_images'] = [val for idx, val in enumerate(each_query_result['selected_images']) if idx not in each_query_pos_remove_list]
...
def check_command_line_args(_args):
    ...
    if micYorN in ['N', 'n']:
        if not os.path.isfile(wavlocfile):
            return 30, f'Error in CLA : If you do not want mic input then the "-file_wav_loc_for_stt" parameter MUST be an existing file', None
    
    return 0, None, [micYorN, save_wavs_here, wavlocfile, opfileallposinfo, logfileloc]
...
def execute_control_logic(args):
    ...
    cla_names = ('-micYorN', '-sw_here', '-file4STT', '-opfileallposinfo', '-oplogfilelocation')
    ...
    micYorN, save_wavs_here, wavlocfile, opfileallposinfo, logfileloc = returned_check_cla_returned_list
    ...
    if micYorN in ['Y', 'y']:
        #######################################################################
        ## STAGE 1 :: MIC INPUT LOGIC
        #######################################################################
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"    STAGE 1)  STARTING EXECUTION OF MIC INPUT LOGIC                ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
        ...
        mic_logic_RC, mic_logic_msg, mic_module_results = mic_input_save_as_wav_functionality(save_wavs_here, wavlocfile, DEBUG_SWITCH)
        myStr = "\n".join([
            f"\nAfter MIC INPUT logic execution:",
            f"mic_logic_RC = {mic_logic_RC}",
            f"mic_logic_msg = {mic_logic_msg}",
            f"mic_module_results = {mic_module_results}\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
    else:
        ## user wants processing to start without mic input and directly pick up the file specified
        mic_logic_RC, mic_logic_msg, mic_module_results = 0, None, None
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"   BYPASSED EXECUTION OF MIC INPUT LOGIC - DIRECTLY NEXT STAGE     ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
    ...
    if mic_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"    STAGE 2)  STARTING EXECUTION OF STT LOGIC                      ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
        ...
        myStr = "\n".join([
            ...
            f"stt_logic_RC = {stt_logic_RC}",
            f"stt_logic_msg = {stt_logic_msg}",
            f"stt_module_results = {stt_module_results}\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
    else:
        myStr = "\n".join([
            f"\n\nFATAL ERROR: MIC LOGIC logic failed. Return code = {mic_logic_RC}",
            f"\tMessage = {mic_logic_msg}",
    ...
    #######################################################################
    ## STAGE 3 :: ID KEY ELEMENTS LOGIC
    #######################################################################
    if stt_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"    STAGE 3)  STARTING EXECUTION OF IDENTIFY KEYWORDS LOGIC        ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
        ...
        myStr = "\n".join([
            ...
            f"\n\nPremature Exit with Exit Code = 100.\n\n"
            ])
        print_and_log(myStr, "error")
        myStr = None
        exit(100)
    ...
    #######################################################################
    ## STAGE 4 :: QUERY NEO4J FOR IMAGES LOGIC
    #######################################################################
    if id_elements_logic_RC == 0:
        ...
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"    STAGE 4)  STARTING EXECUTION OF QUERY NEO4J LOGIC              ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
    ...
    #######################################################################
    ## STAGE 5 :: GUI FOR IMAGE SELECTION JUST AFTER NEO4J QUERY LOGIC
    #######################################################################
    if query_neo4j_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"    STAGE 5)  STARTING EXECUTION OF IMAGE SELECTION VIA GUI        ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
    ...
    #######################################################################
    ## STAGE 5-2 :: MAKE SURE ALL IMAGES FOR ALL QUERIES HAVE NOT BEEN DESLECTED
    #######################################################################
    ...
    #######################################################################
    ## STAGE 6 :: IMAGE CAPTIONING INFERENCE LOGIC
    #######################################################################
    ## do the auto caption block logic processing on the images selected in previous stage - errors checked already
    myStr = "\n".join([
        f"\n\n-------------------------------------------------------------------",
        f"-------------------------------------------------------------------",
        f"    STAGE 6)  STARTING EXECUTION OF IMAGE CAPTIONING               ",
        f"-------------------------------------------------------------------",
        f"-------------------------------------------------------------------\n\n"
        ])
    print_and_log(myStr, 'info')
    myStr = None
    img_cap_inference_logic_RC, img_cap_inference_logic_msg, img_cap_inference_module_results = img_cap_inference_functionality(id_elements_module_results, gui_candidate_image_selection_module_results, IMGCAP_INFERENCE_BEAM_WIDTH, DEBUG_SWITCH)
    myStr = "\n".join([
        f"\nAfter IMAGE CAPTIONING logic execution:",
        f"img_cap_inference_logic_RC = {img_cap_inference_logic_RC}",
        f"img_cap_inference_logic_msg = {img_cap_inference_logic_msg}",
        f"img_cap_inference_module_results = {img_cap_inference_module_results}\n"
        ])
    print_and_log(myStr, "info")
    myStr = None

    #######################################################################
    ## STAGE 7 :: GUI FOR EDITING CAPTIONS AND/OR DESELECTION OF IMAGES LOGIC
    #######################################################################
    ## gui display the images and their captions
    if img_cap_inference_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------------------",
            f"-------------------------------------------------------------------------------",
            f"    STAGE 7)  STARTING EXECUTION OF GUI DISPLAY fOR IMAGE CAPTIONING RESULTS   ",
            f"-------------------------------------------------------------------------------",
            f"-------------------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
        DEBUG_SWITCH = False
        gui_img_cap_show_results_logic_RC, gui_img_cap_show_results_logic_msg, gui_img_cap_module_results = gui_img_cap_show_gui_functionality(img_cap_inference_module_results, DEBUG_SWITCH)
        myStr = "\n".join([
        ...
        myStr = "\n".join([
            f"\n\nFATAL ERROR: Image Captioning Inference logic failed. Return code = {img_cap_inference_logic_RC}",
            f"\tMessage = {img_cap_inference_logic_msg}",
    ...
    #######################################################################
    ## STAGE 8 :: STORY GENERATION LOGIC  -- PENDING
    #######################################################################
