...
class c_micStt_wnd_grid_window:
    def __init__(self, _DEBUG_SWITCH, _root, _micYorN, _save_wav_here, _wavlocfile, _ds_infs_arr, _wav_files_arr):
        ...
        self.btn_confirm_done = tk.Button(
            ...
            text=f"Click to CONFIRM - will action only if ALL inferences are done",
            bg="yellow", fg="black",
            borderwidth=10,
            relief=tk.RAISED,
    ...
    def mic_stt_play_audio_file(self, _pos):
        ...
        if file2play != r'---':
            ## change characteristics of widget to indicate playing file
            bkup_text = self.grid_buttons_arr[0][_pos * 2]["text"]
            self.grid_buttons_arr[0][_pos * 2].configure(text=f"Playing", bg="blue", fg="white")
            ...
            ## restore characteristics of widget to indicate done playing file
            self.grid_buttons_arr[0][_pos * 2].configure(text=bkup_text, bg="yellow", fg="black")
        else:
    ...
    def mic_stt_rec_audio_file(self, _pos):
        ## change characteristics of widget to indicate recording file
        bkup_text = self.grid_buttons_arr[0][_pos * 2 + 1]["text"]
        self.grid_buttons_arr[0][_pos * 2 + 1].configure(text=f"Recording", bg="blue", fg="white")
        ...
        ## change characteristics of widget to indicate recording file
        self.grid_buttons_arr[0][_pos * 2 + 1].configure(text=bkup_text, bg="yellow", fg="black")
    ...
    def mic_stt_perform_inference(self, _pos):
        ...
        ## dummy done
        inf_output = f"some dummy output inserted for file {_pos + 1}"
        self.grid_buttons_arr[_pos + 5][1]["text"] = inf_output
        self.ds_infs_arr[_pos] = inf_output
    ...
    def micStt_do_confirm_processing(self):
        ...
        ## none of the inferences should have the default initial values now
        for inf_val in self.ds_infs_arr:
            if inf_val == "---":
                myStr = f"\nPremature click of Confirm button...no action taken.\n"
                print_and_log(myStr, "info")
                myStr = None
                return
