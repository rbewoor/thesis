...
def query_neo4j_db(_each_inner_list, _in_limit):
    ...
    try:
        ...
        if len(_each_inner_list) == 3:
            ...
            ## possible because user deselected all the keywords - but this should never execute as logic will end earlier in processing
            result = None
        else:
            myStr = "\n".join([
...
def execute_control_logic(args):
    ...
    if id_elements_logic_RC == 0:
        myStr = f"After user selection of candidate keywords logic, RC was = 0, and results =\n{id_elements_module_results}\n\n"
        print_and_log(myStr, 'info')
        myStr = None
