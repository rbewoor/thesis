...
class c_micSttRootWindow():
    def __init__(self, _DEBUG_SWITCH, _micYorN, _save_wav_here, _wavlocfile, _ds_infs_arr, _wav_files_arr):        
        ...
        self.msg_instructions_for_root = "".join([
            ...
            f"\n\t       Important: Depending on the way you ran the program, you may record from mic."
            f"\n",
            f"\n\t2.  If microphone input IS REQUIRED, in the next window:",
            f"\n\t       * You MUST record three sentences if running with Mic input flag enabled.",
            f"\n\t       * Click the Record button and speak to save it as a Wav file.",
            f"\n\t       * Click the Play   button to listen to your recording.",
            f"\n\t       * Click on the Inference button for each file to do the Speech to Text",
            f"\n\t               processing and see its transcription.",
            f"\n\t       * If a particular recording or its transcription is unsatisfactory, then you may",
            f"\n\t               record that file again and runs its STT processing.",
            f"\n",
            f"\n\t3.  If NO microphone input is required, in the next window:",
            f"\n\t       * You MUST have provided an input file containing the three pre-recorded",
            f"\n\t               wav files.",
            f"\n\t       * Click on the Inference button for each file to do the Speech to Text",
            f"\n\t               processing and see its transcription.",
            f"\n",
...
class c_micStt_wnd_grid_window:
    def __init__(self, _DEBUG_SWITCH, _root, _micYorN, _save_wav_here, _wavlocfile, _ds_infs_arr, _wav_files_arr):
        ...
        ## button for selection confirm - disabled by default
        self.btn_confirm_selections = tk.Button(
            ...
            relief=tk.FLAT,
            state=tk.DISABLED,
            command=self.micStt_do_confirm_selections_processing
            )
        self.btn_confirm_selections.configure(
            width=( len(self.btn_confirm_selections["text"]) + 10 ),
            height=3
            )
        ...
        for r_idx in range(1, 4):
            ...
            for c_idx in range(2):
                if c_idx == 0:
                    ...
                    ## Label for file path - switch decides whether No data or populate now
                    if self.micYorN == 'n':
                        temp_row_data.append(
                            tk.Label(
                                master=self.wnd_grid,
    ...
    def micStt_do_confirm_selections_processing(self):
        if self.DEBUG_SWITCH:
...
def mic_and_stt_functionality(_micYorN, _save_wav_here, _wavlocfile, _DEBUG_SWITCH):
    ...
    If user unhappy with the recording and/inference, can redo that particular sentence.
    ...
    ## array for the abolute paths to the 3 wav files
    wav_files_arr = list()
    ...
    deepspeech_inferences_arr = list()
...
def execute_control_logic(args):
    ...
    micYorN, save_wav_here, wavlocfile, opfileallposinfo, logfileloc = returned_check_cla_returned_list
    ...
    #######################################################################
    ## STAGE 4 :: GUI FOR IMAGE SELECTION JUST AFTER NEO4J QUERY LOGIC
    #######################################################################
    if query_neo4j_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
    ...
    #######################################################################
    ## STAGE 4-2 :: MAKE SURE ALL IMAGES FOR ALL QUERIES HAVE NOT BEEN DESLECTED
    #######################################################################
    ...
    #######################################################################
    ## STAGE 6 :: GUI FOR EDITING CAPTIONS AND/OR DESELECTION OF IMAGES LOGIC
    #######################################################################
    ...
    #######################################################################
    ## STAGE 7 :: STORY GENERATION LOGIC  -- PENDING
    #######################################################################
