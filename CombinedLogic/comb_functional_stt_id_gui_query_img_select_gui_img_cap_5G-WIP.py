...
## 
##  4) GUI based Deselection of candidate images obtained from Neo4j query stage.
...
## 
##  5) Image captioning on the images selected by the user.
...
##           b) Decoder is setup as coded below and the weights are populated from a locally saved file.
...
##       Outputs:
##           a) One text file:
##              Results of the module logic - a dict consist of data for each sentence. Data consists of:
##                      i)   Key elements selected by user earlier
##                      ii)  Full path of image chosen by user after GUI selection stage done earlier
##                      iii) Image Captioning model inference i.e. the predicted caption
## 
##  6) GUI presentation of the Images and their captions.
##        Show the images and their captions to the user - currently nothing editable and will be passed on AS IS to next stage.
##        Logic:
##           a) One main window for each original input sentence with instructions.
##           b) Subwindow shows the images and their captions.
##           c) Nothing editable for now.
##       Outputs:
##           None
##               
##
...
## -----------------------------------------------------------------------------
## Usage example:
## python3 comb_functional_stt_id_gui_query_img_select_gui_5B-WIP.py -wavlocfile "/home/rohit/PyWDUbuntu/thesis/combined_execution/SttTranscribe/st_1_HM_Rohit.txt" -opfileallposinfo "/home/rohit/PyWDUbuntu/thesis/combined_execution/IdElements/all_words_pos_info_1.txt" -logfileloc "./LOG_comb_functional_stt_id_gui_query_img_select_gui_5B_WIP.LOG"
## -----------------------------------------------------------------------------

## import necessary packages

##   imports of common or generic use modules
...
def stt_transcribe_functionality(_wavlocfile, _DEBUG_SWITCH):
    ...
    msg_instructions_for_root_stt_inference = "".join([
        ...
        f"\n",
        f"\n",
        f"\nBelow are the transcriptions output from the STT model.",
        f"\n",
        f"\n\t1.  This is just for display purposes.",
        f"\n",
        f"\n\t2.  You cannot edit anything and the will be passed as is for downstream processing."
        f"\n",
...
def imag_cap_inference_functionality(_id_elements_module_results, _gui_candidate_image_selection_module_results, _DEBUG_SWITCH):
    """
    DEBUG_SWITCH = False
    ...
    if (not os.path.exists(SAVED_WEIGHTS_PATH) ) or (not os.path.isfile(SAVED_WEIGHTS_PATH )):
        ...
        exit(112)
    """
    ## not needed here in combined execution as these images would have just been accessed in a previous stage
    ...
    for sentence_info in data_for_img_cap_functionality:
        for each_img_info in sentence_info['selected_images']:
            ...
            out_caption = infer_for_caption_one_image(IMG_TO_INFER, MAX_LENGTH_CAPTION, model_CNN_encoder, reloaded_RNN_decoder, wordtoix, ixtoword, _DEBUG_SWITCH)
...
class c_imgCap_wnd_grid_window:
            ...
            for c_idx, button_or_label in enumerate(logical_set_for_image):
                ...
                if c_idx == 0:
                    ...
                    button_or_label.configure(
                        ...
                        state=tk.NORMAL,
                        command=partial(
                            self.imgCap_do_enlarge_edit_button_clicked_processing,
                            r_idx, c_idx,
                            self.DEBUG_SWITCH
                        )
                        )
    ...
    def imgCap_do_enlarge_edit_button_clicked_processing(self, _r_idx, _c_idx, _DEBUG_SWITCH):
        ## toggle button characteristics:
        ##                              Relief         Color around image
        ##       Selected   Image       SUNKEN         Green
        ##       Deselected Image       RAISED         Red
        if _DEBUG_SWITCH:
            print(f"\n\nEnlarge/Edit button clicked :: (r_idx, c_idx) = {_r_idx, _c_idx}\n\n")
        return
...
def imgcap_generic_show_grid_selection_window(_root, _key_elements_info, _selected_imgs_info, _query_num, _imgCap_deselected_positions, _DEBUG_SWITCH):
    print(f"\nEntered the show gird selection window for img captioning\n")
...
class c_queryImgCapGuiOneQuery():
    def __init__(self, _DEBUG_SWITCH, _query_num, _key_elements_info, _selected_imgs_info, _imgCap_deselected_positions):
        ...
        self.msg_instructions_for_root = "".join([
            ...
            f"\n",
            f"\n",
            f"\nThis is the main control window for captions for all selected candidate images for a particular query.",
            f"\n",
            f"\n\t1.  Click on the button to proceed to display the a grid of images and the captions.",
            f"\n\t       Important: If there are no images to display you cannot click to proceed to the"
            f"\n\t                  grid selection window. Simply close this window and the next query"
            f"\n\t                  selections process will begin."
            f"\n",
            f"\n\t2.  There, you can click the EDIT button to change the caption.",
            f"\n\t       This button will change to DONE button.",
            f"\n\t       Once caption is edited, click DONE to save the change.",
            f"\n",
            f"\n\t3.  Once you are ready to move on to the next query results, click the CONFIRM button.", 
            f"\n",
            f"\n\t4.  You can Enlarge an image to inspect it more closely while deciding to edit the caption.",
            f"\n\t       The image will open in a new window which you may close once done inspecting.",
            f"\n"
        ])
...
## ********** -------------  **************** ------------------ ****************
    ...
    if False:
        ...
        ## data from ID Key Elements selection stage
        id_elements_module_results = [[], ['car'], ['person', 'truck']]
        ...
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"  STARTING EXECUTION OF IMAGE CAPTIONING                           ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, 'info')
        myStr = None
        img_cap_inference_logic_RC, img_cap_inference_logic_msg, img_cap_inference_module_results = imag_cap_inference_functionality(id_elements_module_results, gui_candidate_image_selection_module_results, DEBUG_SWITCH)
        myStr = "\n".join([
...
## ********** -------------  **************** ------------------ ****************
    ...
    img_cap_inference_module_results = [
        {'key_elements': [], 'selected_images': []},
        {
            'key_elements': ['car'],
            'selected_images': [
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000033825.jpg', 'man standing on top of skateboard in front of building'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000155796.jpg', 'street sign with cars and cars on the side of it'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000224207.jpg', 'man riding bike down street with an umbrella'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000313777.jpg', 'train traveling down train tracks next to train station']]
            },
        {
            'key_elements': ['person', 'truck'],
            'selected_images': [
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000169542.jpg', 'truck with truck and truck on the side of it'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000449668.jpg', 'fire hydrant is parked in the middle of the street'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000518174.jpg', 'an airport with luggage and luggage on the tarmac'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000361201.jpg', 'street sign with cars and cars on it'],
                ['/media/rohit/DATA/EverythingD/01SRH-BDBA Acads/Thesis/StoryGenerator/Data/COCO_test2017_41k/test2017/000000304424.jpg', 'train is parked on the side of the road']]
            }
        ]
    ...
    if img_cap_inference_logic_RC == 0:
        myStr = "\n".join([
            f"\n\n-------------------------------------------------------------------",
            f"-------------------------------------------------------------------",
            f"  STARTING EXECUTION OF GUI DISPLAY fOR IMAGE CAPTIONING RESULTS   ",
            f"-------------------------------------------------------------------",
            f"-------------------------------------------------------------------\n\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
        DEBUG_SWITCH = False
        gui_img_cap_show_results_logic_RC, gui_img_cap_show_results_logic_msg, gui_img_cap_show_results_module_results = gui_img_cap_show_gui_functionality(img_cap_inference_module_results, DEBUG_SWITCH)
        myStr = "\n".join([
            ...
            f"gui_img_cap_show_results_logic_msg = {gui_img_cap_show_results_logic_msg}",
            f"gui_img_cap_show_results_module_results = {gui_img_cap_show_results_module_results}\n"
            ])
        print_and_log(myStr, "info")
        myStr = None
    else:
        ...
        print(f"\n\nPPremature Exit with Exit Code = 450\n\n")
        exit(450)
