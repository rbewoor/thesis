...
class c_micStt_wnd_grid_window:
    ...
    def mic_stt_play_audio_file(self, _pos):
        ...
        if file2play != r'---':
            ...
            stream.close()
            p.terminate()
            myStr = f"\nFinished playing file {_pos + 1}\n"
            print_and_log(myStr, "info")
            myStr = None
            ## change user message
            self.change_label_user_message_text(f"Finished playing file {_pos + 1}.", _delay=2000)
        else:
            myStr = f"\nCannot play as file path was HYPHENS.\n"
            print_and_log(myStr, "info")
            myStr = None
            ## change user message
            self.change_label_user_message_text(f"Cannot play as file path was HYPHENS.", _delay=2000)
