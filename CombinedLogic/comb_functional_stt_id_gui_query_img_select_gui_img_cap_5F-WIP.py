...
class c_imgCap_wnd_grid_window:
    def __init__(self, _root, _key_elements_info, _selected_imgs_info, _query_num, _imgCap_deselected_positions, _DEBUG_SWITCH):
        ...
        ##       NOT for Confirm button and Label for current count.
        ##           They will be referenced by using row count and indexing further down automatically.
        ## ENSURE #rows is multiple of 3 coz the logic in window grid array holding buttons/labels expects this
        self.n_rows = 15
        ...
        for r_idx in range(self.n_rows):
            ...
            if r_idx % 3 == 0:
                ...
                ## associated enlarge button - pos 5 in inner list
                temp_row_data.append(
                    tk.Button(
                        master=self.wnd_grid,
                        text="Enlarge",
                ...
                ## associated edit caption button - pos 6 in inner list
                temp_row_data.append(
                    tk.Button(
                        master=self.wnd_grid,
                        text="Edit Caption",
        ...
        ##     thumbnail button, path label, caption label, enlarge button, edit caption
            ...
            for c_idx, button_or_label in enumerate(logical_set_for_image):
                ...
                if c_idx == 0:
                    ...
                    button_or_label.configure(
                        ...
                        state=tk.NORMAL,
                        command=partial(
                            self.imgCap_do_enlarge_button_clicked_processing,
                            r_idx, c_idx,
                            self.DEBUG_SWITCH
                        )
                        )
                    ...
                    button_or_label.configure(
                        ...
                        state=tk.NORMAL,
                        command=partial(
                            self.imgCap_do_edit_button_clicked_processing,
                            r_idx, c_idx,
                            self.DEBUG_SWITCH
                            )
                        )
    ...
    def imgCap_do_enlarge_button_clicked_processing(self, _r_idx, _c_idx, _DEBUG_SWITCH):
        ## toggle button characteristics:
        ##                              Relief         Color around image
        ##       Selected   Image       SUNKEN         Green
        ##       Deselected Image       RAISED         Red
        if _DEBUG_SWITCH:
            print(f"\n\nEnlarge button clicked :: (r_idx, c_idx) = {_r_idx, _c_idx}\n\n")
    
    def imgCap_do_edit_button_clicked_processing(self, _r_idx, _c_idx, _DEBUG_SWITCH):
        ## toggle button characteristics:
        ##                              Relief         Color around image
        ##       Selected   Image       SUNKEN         Green
        ##       Deselected Image       RAISED         Red
        if _DEBUG_SWITCH:
            print(f"\n\nEdit button clicked :: (r_idx, c_idx) = {_r_idx, _c_idx}\n\n")
    ...
    def imgCap_do_confirm_selections_processing(self):
        ...
        for r_idx, logical_set_for_image in enumerate(self.grid_buttons_arr):
            img_button_info, _, _, _, _, _, _ = logical_set_for_image ## only interested in the first entry
